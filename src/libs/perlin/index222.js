...
class Index {
	...
	interpolation(f) {
		switch (typeof f) {
			case 'string':
				if ((f + 'Interpolate') in Index) {
					this._interpolate = Index[f + 'Interpolate'];
				} else {
	...
	static generateSmoothNoise(aBaseNoise, nOctave) {
		...
		let interpolate = Index.cosineInterpolate;
		for (let x, y = 0; y < h; ++y) {
	...
	static generatePerlinNoise(aBaseNoise, nOctaveCount) {
		...
		for (let i = 0; i < nOctaveCount; ++i) {
			aSmoothNoise[i] = Index.generateSmoothNoise(aBaseNoise, i);
		}
	...
	static getPointHash(x, y) {
		let xh = Index.hash(x).toString().split('');
		let yh = Index.hash(y).toString().split('');
	...
	generate(x, y, callbacks) {
		...
		let cached = this._cache.load(x, y);
		if (cached) {
			return cached;
		}
		...
		const gwn = (xg, yg) => {
			...
			let nSeed = Index.getPointHash(xg, yg);
		...
		let a1 = merge33(a0);
		let a2 = Index.generatePerlinNoise(a1, this._octaves);
		let a3 = extract33(a2);
		if (perlin) {
			a3 = perlin(x, y, a3);
		}
		this._cache.store(x, y, a3);
		return a3;
	}
